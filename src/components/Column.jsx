...
const Column = ({ id, box, setBox }) => {
    ...
    //delete task
    const deleteTask = (id) => {
        setBox(box.filter((task) => task.id !== id));
    };

    // Handles adding a task
    const addTask = () => {
        if (taskTitle.trim() === "") return;
        setBox([...box, { id: Date.now().toString(), title: taskTitle, category: id }]);
        setTaskTitle("");
        setIsAdding(false);
    };
    ...
    const updateTask = (id, title) => {
        const updatedTasks = box.map((task) => (task.id === id ? { ...task, title } : task));
        setBox(updatedTasks);
    }
    ...
    return (
        <div ref={setNodeRef} className={`w-1/3 bg-gray-800 p-4 rounded-lg shadow-md ${isOver ? "border-2 border-green-500" : ""}`}>
            ...
            {box?.map((item) => (
                <SortableItem key={item.id} id={item.id} title={item.title} category={item.category} deleteTask={deleteTask} updateTask={updateTask}/>
            ))}
